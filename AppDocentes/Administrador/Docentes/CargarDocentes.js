$(document).ready(function () {
    ...
    $('#btnSubirDocentes').click(function () {
        MandarDocentes();
    });
    ...
    function MandarDocentes() {
        ...
        $(tmpExcel).each(function () {
            ...
            datosD.Docentes.push(obj);
        });

        datosD.UsuarioId = sessionStorage.getItem("Usuario");
        $.ajax({
            type: "POST",
            url: "../../Api/Docente/GuardarDocentes",
            ...
            success: function (data) {
                if (data.length == 0) {
                    $('#Load').modal('hide');
                    alertify.alert("Docentes Guardadoss", function(){
                        location.reload();
                    });
                    
                } else {
                    alertify.alert("Error Al Guardar </br> Los siguientes docentes no se guardaron.");
                    $('#Load').modal('hide');
                    tmpExcel = data;
    ...
    function CrearTabla() {
        tblRegistros = $('#tblDocentes').dataTable({
            "aaData": tmpExcel,
            "aoColumns": [
                { "mDataProp": "Nombre" },
                { "mDataProp": "Paterno" },
